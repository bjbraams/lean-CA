...
(`k : ℕ`, arbitrary integer exponents `g k`). -/
theorem circleIntegral_zpow_mul_tsum_zpow_eq (d : ℕ → ℂ) (g : ℕ → ℤ) (R : ℝ) (hR : 0 < R)
  ...
  have hzpow (m : ℤ) : ContinuousOn (fun w : ℂ ↦ w ^ m) (sphere 0 R) :=
    (continuousOn_zpow₀ m).mono fun w hw ↦ ne_zero_of_mem_sphere_zero hR hw
  simp_rw [← tsum_mul_left]
  rw [circleIntegral_tsum_of_norm_le hR.le (f := fun k w ↦ w ^ e * (d k * w ^ (g k)))
    (u := fun k ↦ R ^ e * (‖d k‖ * R ^ (g k)))
    (fun k ↦ (hzpow e).mul (continuousOn_const.mul (hzpow (g k)))) (hd.mul_left _)
    fun k w hw ↦ by rw [norm_mul, norm_zpow, norm_mul, norm_zpow, mem_sphere_zero_iff_norm.mp hw]]
  congr 1 with k
  rw [← circleIntegral.integral_const_mul]
  refine circleIntegral.integral_congr hR.le fun w hw ↦ ?_
  rw [zpow_add₀ (ne_zero_of_mem_sphere_zero hR hw)]
  ring
